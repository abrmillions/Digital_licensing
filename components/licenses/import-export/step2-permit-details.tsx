...
export function ImportExportStep2({ data, updateData, onNext, onBack }: Step2Props) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (data.purposeOfImport && data.purposeOfImport.length > 500) {
      setPurposeError("Purpose of import/export must be 500 characters or less")
      return
    }
      ...
      <div className="grid md:grid-cols-2 gap-4">
        ...
        <div className="space-y-2 md:col-span-2">
          ...
          <Textarea
            ...
            onChange={(e) => {
              updateData({ purposeOfImport: e.target.value })
              setPurposeError(null)
            }}
            placeholder="Describe the purpose and intended use of the equipment/materials"
            rows={4}
            className={purposeError ? "border-destructive focus-visible:ring-destructive" : ""}
            required
          />
          {purposeError && (
            <p className="text-xs text-destructive mt-1 font-medium">{purposeError}</p>
          )}
        </div>
      </div>
