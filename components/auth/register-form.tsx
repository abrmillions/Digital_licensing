...
export function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.phone) {
      const ethPhoneRegex = /^\+251\d{9}$/;
      if (!ethPhoneRegex.test(formData.phone)) {
        setError("Invalid Ethiopian phone number. Must start with +251 followed by 9 digits (e.g., +251911223344).");
        return;
      }
    }
