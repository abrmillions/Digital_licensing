...
export const authApi = {
  register: async (data: any) => {
    ...
    const payload: any = {
      ...
      first_name:
        data.first_name || data.firstName || data.fullName || data.fullname,
      fullName:
        data.fullName || data.first_name || data.firstName || data.fullname,
      phone: data.phone,
    ...
    try {
      ...
      try {
        // Use djangoApiRequest for login to benefit from proxying and CORS handling
        const tokens = await djangoApiRequest<{
          access: string;
          refresh: string;
        ...
        try {
          ...
          const mappedUser = {
            ...
            profilePhoto: u.profile_photo
              ? u.profile_photo.startsWith("http")
                ? u.profile_photo
                : `${DJANGO_API_URL}${u.profile_photo}`
              : null,
            role: (u.role || (u.is_staff ? "Admin" : "User")),
          };
          if (typeof window !== "undefined")
            localStorage.setItem("clms_user", JSON.stringify(mappedUser));
        } catch (e) {
          console.warn("[clms] Failed to fetch user after register:", e);
        }
      } catch (e) {
        console.warn("[clms] Login after register failed:", e);
      }
      return user;
    } catch (err: any) {
  ...
  login: async (email: string, password: string) => {
    ...
    try {
      ...
      if (!direct.ok) {
        ...
        const err: any = new Error(
          `API Error: ${direct.status} ${direct.statusText}`,
        );
      ...
      const isNetworkFail =
        /failed to fetch/i.test(msg) || e?.name === "TypeError";
      ...
      if (shouldFallback) {
        ...
        try {
          response = await djangoApiRequest<{
            access: string;
            refresh: string;
          }>("/api/users/token/", {
            method: "POST",
          ...
          try {
            ...
            // Final fallback 3: same-origin proxy with form-encoded
            const formBody = new URLSearchParams({
              email,
              username: email,
              password,
    ...
    try {
      ...
      console.debug("[clms][authApi] login token response:", response);
    } catch (e) {
        ...
        const mappedUser = {
          ...
          profilePhoto: user.profile_photo
            ? user.profile_photo.startsWith("http")
              ? user.profile_photo
              : `${DJANGO_API_URL}${user.profile_photo}`
            : null,
  ...
  checkEmail: async (email: string) => {
    ...
    return djangoApiRequest(url, { skipAuth: true, suppressLog: true });
  },
  ...
  changePassword: async (
    ...
    return djangoApiRequest(
      `${DJANGO_ENDPOINTS.auth.login.replace("/token/", "/change_password/")}`,
      {
        method: "POST",
        body: JSON.stringify({
...
export const licensesApi = {
    ...
    try {
      ...
      try {
        ...
        if (isActiveExisting) {
          ...
          try {
            const renewals = await djangoApiRequest<any[]>(
              DJANGO_ENDPOINTS.licenses.renewalsList,
              { suppressLog: true },
            );
            const match = (renewals || []).find((r: any) => {
              const licId = String(
                r?.license_id || r?.licenseId || r?.license?.id || "",
              ).trim();
          ...
          try {
            const apps = await djangoApiRequest<any[]>(
              DJANGO_ENDPOINTS.applications.list,
              { suppressLog: true },
            );
            const candidate = (apps || []).find((a: any) => {
              ...
              const kind = String(
                a?.type || a?.category || a?.purpose || "",
              ).toLowerCase();
              const looksRenewal = kind.includes("renew");
              const pendingish =
                !status ||
                ["pending", "in_progress", "active", "open"].includes(status);
              return (
                licId &&
                String(licId) === String(id) &&
                (looksRenewal || pendingish)
              );
            });
            const appId = String(
              candidate?.id || candidate?.application?.id || "",
            );
            if (appId) {
              return { id: appId, application: { id: appId } };
            }
  ...
  approveRenewal: async (id: string) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.licenses.renewalApprove(id), {
      method: "PATCH",
    });
  },
  rejectRenewal: async (id: string, reason?: string) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.licenses.renewalReject(id), {
      method: "PATCH",
  ...
  verify: async (licenseNumber: string) => {
    return djangoApiRequest(
      `${DJANGO_ENDPOINTS.licenses.verify}?license_number=${licenseNumber}`,
    );
  },
  ...
  download: async (id: string) => {
    try {
      return await djangoApiRequest(DJANGO_ENDPOINTS.licenses.download(id), {
        suppressLog: true,
      });
    } catch (e: any) {
      ...
      const msg =
        (e?.error?.detail && String(e.error.detail).trim()) ||
        (e?.message && String(e.message).trim()) ||
        `License download failed (${e?.status || "error"})`;
      const ne: any = new Error(msg);
      ne.status = e?.status || 0;
      ne.error = { detail: msg };
      throw ne;
...
export const notificationsApi = {
  list: async () => {
    return djangoApiRequest(`${DJANGO_ENDPOINTS.applications.base}notifications/`, {
      suppressLog: true,
    });
  },
  markRead: async (id: number) => {
    return djangoApiRequest(`${DJANGO_ENDPOINTS.applications.base}notifications/${id}/mark-read/`, {
      method: "POST",
    });
  },
  markAllRead: async () => {
    return djangoApiRequest(`${DJANGO_ENDPOINTS.applications.base}notifications/mark-all-read/`, {
      method: "POST",
    });
  },
};
...
export const applicationsApi = {
  ...
  create: async (data: any) => {
    try {
      const endpoint =
        typeof window !== "undefined"
          ? "/api/system/settings/"
          : DJANGO_ENDPOINTS.system.settings;
      const sys = await djangoApiRequest(endpoint, { suppressLog: true });
      const isMaintenance = !!(
        sys &&
        (sys.maintenance_mode ?? sys.maintenanceMode)
      );
      if (isMaintenance) {
        const e: any = new Error(
          "Maintenance in Progress: submissions are temporarily disabled.",
        );
        e.status = 503;
        e.error = {
          detail:
            "The system is currently in Maintenance in Progress updating. Please try again later.",
        };
        throw e;
      }
    } catch (preCheckErr: any) {
      const s = preCheckErr?.status || 0;
      if (s === 503) throw preCheckErr;
    }
    // If any photo fields are File objects, submit as multipart/form-data
    const photoKeys = [
      "profile_photo",
      "professional_photo",
      "lead_representative_photo",
  ...
  downloadDocuments: async (id: string) => {
    try {
      ...
      const msg =
        (e?.error?.detail && String(e.error.detail).trim()) ||
        (e?.message && String(e.message).trim()) ||
        `Documents download failed (${e?.status || "error"})`;
  ...
  getStats: async () => {
    return djangoApiRequest(DJANGO_ENDPOINTS.applications.stats, {
      suppressLog: true,
    });
  },
};
...
export const partnershipsApi = {
  ...
  approve: async (id: string): Promise<Partnership> => {
    return djangoApiRequest(DJANGO_ENDPOINTS.partnerships.approve(id), {
      method: "POST",
    });
  },

  reject: async (id: string): Promise<Partnership> => {
    return djangoApiRequest(DJANGO_ENDPOINTS.partnerships.reject(id), {
      method: "POST",
    });
  },
  ...
  publicVerify: async (id: string): Promise<any> => {
    ...
    try {
      ...
      const isCertLike =
        /^CP-\d{4}-[A-Z0-9]{6}$/i.test(raw) || /^PCERT-/i.test(raw);
      if (!isUuid && isCertLike) {
        return await djangoApiRequest(
          DJANGO_ENDPOINTS.partnerships.verifyCert(raw),
          { skipAuth: true },
        );
      }
      try {
  ...
  verifyDocuments: async (id: string) => {
    return djangoApiRequest(`${DJANGO_ENDPOINTS.partnerships.detail(id)}verify_documents/`, { 
      method: 'POST',
      timeout: 120000 
    })
  },
  ...
  verifyDocument: async (docId: string) => {
    return djangoApiRequest(`${DJANGO_API_URL}/api/verify-pdoc/`, {
      method: 'POST',
...
export const paymentsApi = {
  ...
  initializeChapa: async (data: {
    ...
    return djangoApiRequest(DJANGO_ENDPOINTS.payments.chapaCreate, {
      method: "POST",
      body: JSON.stringify(data),
    });
  },
  verifyChapa: async (txRef: string) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.payments.chapaVerify(txRef));
  },
};
...
export const vehiclesApi = {
  ...
  verifyDocuments: async (id: string) => {
    return djangoApiRequest(
      `${DJANGO_ENDPOINTS.vehicles.detail(id)}verify_documents/`,
      { method: "POST", timeout: 120000 },
    );
  },

  verifyDocument: async (docId: string) => {
    return djangoApiRequest(`${DJANGO_API_URL}/api/verify-vdoc/`, {
      method: 'POST',
      body: JSON.stringify({ document_id: docId }),
      timeout: 120000
    })
...
export const analyticsApi = {
  getDashboard: async (range?: string) => {
    const url = range 
      ? `${DJANGO_ENDPOINTS.analytics.dashboard}?range=${range}`
      : DJANGO_ENDPOINTS.analytics.dashboard;
    return djangoApiRequest(url);
  },
...
export const applicationsApiEx = {
  verifyDocuments: async (id: string) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.applications.verifyDocuments(id), {
      method: "POST",
      timeout: 120000,
    });
  },
...
export const settingsApi = {
  get: async () => {
    ...
    return {
      ...
      emailDebugMode: data.email_debug_mode,
      smsNotifications: data.sms_notifications,
      autoApproval: data.auto_approval,
      maintenanceMode: data.maintenance_mode,
      documentVerificationEnabled: data.document_verification_enabled,
      sessionTimeout: data.session_timeout,
      maxLoginAttempts: data.max_login_attempts,
      passwordMinLength: data.password_min_length,
      deepseekApiKey: data.deepseek_api_key || data.deepseekApiKey,
      deepseekModel: data.deepseek_model || data.deepseekModel,
      deepseekTimeout: data.deepseek_timeout || data.deepseekTimeout,
      deepseekMaxRetries: data.deepseek_max_retries || data.deepseekMaxRetries,
      ocrLanguage: data.ocr_language || data.ocrLanguage,
      ocrTimeout: data.ocr_timeout || data.ocrTimeout,
      notificationTemplate: data.notification_template,
      openrouterApiKey: data.openrouter_api_key || data.openrouterApiKey,
      openrouterModel: data.openrouter_model || data.openrouterModel,
      openrouterTimeout: data.openrouter_timeout || data.openrouterTimeout,
      geminiApiKey: data.gemini_api_key || data.geminiApiKey,
      geminiModel: data.gemini_model || data.geminiModel,
      geminiTimeout: data.gemini_timeout || data.geminiTimeout,
      preferredAiProvider: data.preferred_ai_provider || data.preferredAiProvider,
      ...
      ipWhitelistEnabled: data.ip_whitelist_enabled,
      adminIpWhitelist: data.admin_ip_whitelist,
      verifiedOnlyTrue: data.verified_only_true || data.verifiedOnlyTrue,
  ...
  update: async (settings: any) => {
    ...
    const payload = {
      ...
      email_debug_mode: settings.emailDebugMode,
      sms_notifications: settings.smsNotifications,
      auto_approval: settings.autoApproval,
      maintenance_mode: settings.maintenanceMode,
      document_verification_enabled: settings.documentVerificationEnabled,
      ...
      deepseek_timeout: settings.deepseekTimeout,
      deepseek_max_retries: settings.deepseekMaxRetries,
      ocr_language: settings.ocrLanguage,
      ocr_timeout: settings.ocrTimeout,
      ...
      openrouter_timeout: settings.openrouterTimeout,
      gemini_api_key: settings.geminiApiKey,
      gemini_model: settings.geminiModel,
      gemini_timeout: settings.geminiTimeout,
      preferred_ai_provider: settings.preferredAiProvider,
      smtp_host: settings.smtpHost,
      smtp_port: settings.smtpPort,
      smtp_user: settings.smtpUser,
      smtp_password: settings.smtpPassword,
      smtp_use_tls: settings.smtpUseTls,
      two_factor_authentication_enabled: settings.twoFactorAuthenticationEnabled,
      ip_whitelist_enabled: settings.ipWhitelistEnabled,
      admin_ip_whitelist: settings.adminIpWhitelist,
    };
    const data = await djangoApiRequest(endpoint, {
      method: "PATCH",
      body: JSON.stringify(payload),
      suppressLog: true,
    });
...
export const contactApi = {
  listMessages: async () => {
    return await djangoApiRequest(DJANGO_ENDPOINTS.contact.messages, { method: "GET" });
  },
  createMessage: async (payload: any) => {
    return await djangoApiRequest(DJANGO_ENDPOINTS.contact.messages, {
      method: "POST",
      body: JSON.stringify(payload),
    });
  },
  getMessageDetail: async (id: number | string) => {
    return await djangoApiRequest(DJANGO_ENDPOINTS.contact.messageDetail(id), { method: "GET" });
  },
  updateMessage: async (id: number | string, payload: any) => {
    return await djangoApiRequest(DJANGO_ENDPOINTS.contact.messageDetail(id), {
      method: "PATCH",
      body: JSON.stringify(payload),
    });
  },
  deleteMessage: async (id: number | string) => {
    return await djangoApiRequest(DJANGO_ENDPOINTS.contact.messageDetail(id), { method: "DELETE" });
  },
  replyToMessage: async (id: number | string, text: string) => {
    return await djangoApiRequest(DJANGO_ENDPOINTS.contact.messageReply(id), {
      method: "POST",
      body: JSON.stringify({ text }),
    });
  },
  updateReply: async (id: number | string, text: string) => {
    return await djangoApiRequest(DJANGO_ENDPOINTS.contact.replyDetail(id), {
      method: "PATCH",
      body: JSON.stringify({ text }),
    });
  },
...
export const companiesApi = {
  list: async (filters?: Record<string, any>) => {
    ...
    return djangoApiRequest(`${DJANGO_ENDPOINTS.companies.list}${query}`);
  },
  getDetail: async (id: string) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.companies.detail(id));
  },
  create: async (data: any) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.companies.create, {
      method: "POST",
      body: JSON.stringify(data),
      suppressLog: true,
    });
  ...
  update: async (id: string, data: any) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.companies.update(id), {
      method: "PATCH",
      body: JSON.stringify(data),
    });
  },
  delete: async (id: string) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.companies.delete(id), {
      method: "DELETE",
    });
  },
};
...
export const documentsApi = {
  list: async (filters?: Record<string, any>) => {
    ...
    try {
      return await djangoApiRequest(
        `${DJANGO_ENDPOINTS.documents.list}?${query}`,
      );
    } catch (e: any) {
      if (e?.status === 404) {
        return [];
  ...
  validatePhoto: async (file: File) => {
    const fd = new FormData();
    fd.append("file", file);
    return djangoApiRequest("/api/documents/validate_photo/", {
      method: "POST",
      body: fd,
      timeout: 120000, // Increase timeout to 2 minutes for AI validation
    });
  },
  ...
  upload: async (
    ...
    try {
      return await djangoApiRequest(DJANGO_ENDPOINTS.documents.upload, {
        method: "POST",
        body: fd,
        timeout: 120000, // Increase timeout for document uploads
        headers: {
          // Let the browser set Content-Type for FormData
        },
      });
      ...
      if (isNetworkFail) {
        const networkError = new Error("Check your internet connection");
      ...
      try {
        ...
        if (isNetworkFail || status === 0) throw e;
        throw e;
      }
    }
  },
  ...
  download: async (id: string) => {
    ...
    return djangoApiRequest<Blob>(detailUrl, {
      method: "GET",
      responseType: "blob",
    });
  },
  ...
  downloadByUrl: async (url: string) => {
    ...
    try {
      if (typeof window !== "undefined" && NEXT_PUBLIC_USE_PROXY) {
        try {
          ...
          if (path.startsWith("/media/")) {
            ...
            return await djangoApiRequest<Blob>(proxied, {
              method: "GET",
              responseType: "blob",
            });
          }
      ...
      return await djangoApiRequest<Blob>(full, {
        method: "GET",
        responseType: "blob",
      });
    } catch (e: any) {
  ...
  verify: async (id: string) => {
    return djangoApiRequest(DJANGO_ENDPOINTS.documents.verify(id), {
      method: "POST",
      timeout: 120000, // 120 seconds for AI verification
    });
  },
};
...
const djangoApi = {
  ...
  verifyLicense: async (
    opts: { licenseNumber?: string; token?: string } | string,
  ) => {
    ...
    return djangoApiRequest(
      `${DJANGO_ENDPOINTS.licenses.verify}${query ? `?${query}` : ""}`,
    );
  },
};
