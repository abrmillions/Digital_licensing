...
export async function generateLicensePDF(license: any) {
  ...
  // 3. Header Section (Issuing Authority & Logo)
  const headerY = borderInset + 12;

  // Formal Issuing Authority Header
  pdf.setFont("times", "bold");
  pdf.setFontSize(11);
  pdf.setTextColor(black.r, black.g, black.b);
  pdf.text("FEDERAL DEMOCRATIC REPUBLIC OF ETHIOPIA", pageWidth / 2, headerY, {
    align: "center",
  });

  pdf.setFontSize(14);
  pdf.text("OROMIA CONSTRUCTION AUTHORITY", pageWidth / 2, headerY + 6, {
    align: "center",
  });
  ...
  let typeStr = (
    ...
    license.type ||
    license.category ||
    (license.data && (license.data.license_type || license.data.licenseType || license.data.category)) ||
    "PROFESSIONAL"
  ).toString();
  ...
  if (
    typeLower.includes("contractor") || 
    typeLower.includes("profile") ||
    typeLower.includes("construction")
  )
    typeStr = "CONTRACTOR LICENSE";
  else if (
    typeLower.includes("import-export") ||
    typeLower.includes("import/export") ||
    typeLower === "company_representative"
  )
    typeStr = "IMPORT/EXPORT LICENSE";
  else if (typeLower.includes("professional"))
    typeStr = "PROFESSIONAL LICENSE";
  else if (typeLower.includes("vehicle"))
    typeStr = "VEHICLE LICENSE";
  else typeStr = typeStr.toUpperCase().replace(/_/g, " ");

  // Handle case where typeStr might already contain the grade info from backend (e.g. "Contractor License - Grade A")
  if (typeLower.includes(" - grade")) {
     // Keep it as is, but uppercase
  ...
  pdf.text(typeStr, pageWidth / 2, logoY + 32, { align: "center" });

  // 4. Main Section (Photo & Name)
  const photoY = logoY + 45;
  const photoW = 45;
  const photoH = 55;
  ...
  // Embed Photo
  const d = (license as any).data || {};
  const photoUrl =
    (license as any).license_photo_base64 ||
    (license as any).licensePhotoBase64 ||
    (license as any).photo_url ||
    (license as any).photoUrl ||
    (license as any).licensePhotoUrl ||
    (license as any).license_photo_url ||
    (license as any).photo ||
    d.photo ||
    d.photo_url ||
    d.photoUrl ||
    d.licensePhoto ||
      ...
      if (typeof photoUrl === "string" && photoUrl.startsWith("data:")) {
        ...
        if (
          ...
          blob = await djangoApiRequest<Blob>(resolvedUrl, {
            responseType: "blob",
            skipAuth: true, // Media files usually don't need auth, and skipping avoids CORS preflight issues
          });
        } else {
  ...
  if (subInfoText) {
    ...
    pdf.text(`${subInfoLabel}${subInfoText}`, pageWidth / 2, textY + 12, {
      align: "center",
    });
  }
  ...
  const holderNameY = subInfoText ? textY + 25 : textY + 12;
  ...
  if (isContractor) {
    ...
    const gVal = subInfoText || "N/A";

    pdf.setFont("times", "italic");
    pdf.setFontSize(10);
    pdf.setTextColor(grey.r, grey.g, grey.b);

    const verificationText = `This Verified Contractor License confirms that "${cVal}" has met the technical, financial, and legal requirements for "${gVal}" General Contracting under the laws of the Federal Democratic Republic of Ethiopia, as verified by the Oromia Construction Authority.`;

    const splitText = pdf.splitTextToSize(
      verificationText,
      pageWidth - (margin + 15) * 2,
    );
    pdf.text(splitText, pageWidth / 2, infoY, { align: "center" });
    infoY += splitText.length * 5 + 8;
  }
  ...
  if (isProfessional) {
    ...
    const posVal = subInfoText || "N/A";
    const verificationText = `This Verified Professional License confirms that "${cleanHolder}" has met the educational and experience requirements for registration as an "${posVal}" under the laws of the Federal Democratic Republic of Ethiopia, as verified by the Oromia Construction Authority.`;
  ...
  if (isImportExport) {
    ...
    const detailVal = subInfoText || "Import/Export";
    const verificationText = `This Verified Import/Export License confirms that "${cleanHolder}" has met the educational and experience requirements for registration as an "${detailVal}" under the laws of the Federal Democratic Republic of Ethiopia, as verified by the Oromia Construction Authority.`;
  ...
  const line1Part1 = `${typeLabel}${finalTypeDisplay}`;
  const line1Part2 = `${companyLabel}${companyVal}`;
  ...
  const catText = `${catLabel}${catVal.toUpperCase()}`;

  pdf.text(catText, pageWidth / 2, row2Y, { align: "center" });

  // Status Badge (Active)
  const badgeY = row2Y + 7;
  ...
  if (qrDataUrl) {
    const qrSize = 35;
    const qrX = margin + 10;
    const qrY = bottomY - qrSize + 12;
    pdf.setFont("times", "normal");
    pdf.setFontSize(10);
    pdf.setTextColor(black.r, black.g, black.b);
    const dateX = qrX;
    const issuedYAboveQR = qrY - 7;
    const expiryYAboveQR = qrY - 1;
  ...
  // 8. Signature & Seal Section
  const sigY = bottomY - 5;
  const sigX = pageWidth / 2;

  // Render Stylized "Handwritten" Signature
  const drawHandwrittenSignature = (x: number, y: number) => {
    pdf.setDrawColor(0, 0, 128); // Classic Blue Ink
    pdf.setLineWidth(0.6);

    // Stylized "A" (First Initial)
    pdf.line(x - 25, y - 2, x - 20, y - 12); // Up
    pdf.line(x - 20, y - 12, x - 15, y - 2); // Down
    pdf.line(x - 23, y - 7, x - 17, y - 7); // Cross

    // Cursive "b" and loops
    pdf.line(x - 15, y - 2, x - 12, y - 10);
    pdf.line(x - 12, y - 10, x - 10, y - 2);
    pdf.line(x - 10, y - 2, x - 5, y - 6);
    pdf.line(x - 5, y - 6, x, y - 2);

    // Middle name initial "M"
    pdf.line(x + 5, y - 2, x + 7, y - 10);
    pdf.line(x + 7, y - 10, x + 9, y - 5);
    pdf.line(x + 9, y - 5, x + 11, y - 10);
    pdf.line(x + 11, y - 10, x + 13, y - 2);

    // Large flourish/underline
    pdf.line(x - 28, y + 1, x + 25, y - 1);
    pdf.line(x + 25, y - 1, x + 15, y + 2);
  };
  ...
  try {
    ...
    if (signatureUrl) {
      ...
      try {
        ...
        drawHandwrittenSignature(sigX, sigY);
      }
    } else {
      // Use the generated "handwritten" signature as the primary look
      drawHandwrittenSignature(sigX, sigY);
    }

    // Official Title below signature
    pdf.setFont("times", "bold");
    pdf.setFontSize(10);
    pdf.setTextColor(black.r, black.g, black.b);
    pdf.text("DIRECTOR GENERAL", pageWidth / 2, sigY + 5, { align: "center" });
    pdf.setFont("times", "normal");
    pdf.setFontSize(8);
    pdf.setTextColor(grey.r, grey.g, grey.b);
    pdf.text("Oromia Construction Authority", pageWidth / 2, sigY + 9, {
      align: "center",
    });
  } catch (e) {
    drawHandwrittenSignature(sigX, sigY);
  ...
  // 9. Official Seal (Right) - Using Provided Seal Image
  const sealSize = 38;
  const sealX = pageWidth - margin - sealSize - 2;
  const sealY = bottomY - sealSize + 10;

  try {
    // For partnership certificates, we prioritize the chapa.png as requested
    // Using a more robust loading method to ensure it appears correctly
    const sealUrl = "/chapa.png";
    ...
    const b64 =
      typeof window !== "undefined"
        ? window.btoa(binary)
        : Buffer.from(bytes).toString("base64");
    const sealDataUrl = `data:image/png;base64,${b64}`;

    pdf.addImage(sealDataUrl, "PNG", sealX, sealY, sealSize, sealSize);
  } catch (e) {
    console.warn(
      "Failed to load official seal /chapa.png, falling back to generated seal",
      e,
    );
    // Fallback: Gold Seal Body (Sawtooth) if image fails
    const fallbackSealX = sealX + sealSize / 2;
    const fallbackSealY = sealY + sealSize / 2;
    const sealR = sealSize / 2;

    pdf.setFillColor(gold.r, gold.g, gold.b);
    pdf.circle(fallbackSealX, fallbackSealY, sealR - 1, "F");

    pdf.setDrawColor(gold.r, gold.g, gold.b);
    pdf.setLineWidth(1.5);
    pdf.setLineDashPattern([1, 1], 0);
    pdf.circle(fallbackSealX, fallbackSealY, sealR, "S");
    pdf.setLineDashPattern([], 0);

    pdf.setTextColor(255, 255, 255);
    pdf.setFont("times", "bold");
    pdf.setFontSize(10);
    pdf.text("APPROVED", fallbackSealX, fallbackSealY + 1, { align: "center" });
  }
...
export async function generateVehicleCertificatePDF(vehicle: any) {
  ...
  try {
    // Official Vehicle Certificate Seal (chapa.png)
    const sealUrl = "/chapa.png";
    const response = await fetch(sealUrl);
    const blob = await response.blob();
    const arrayBuffer = await blob.arrayBuffer();
    ...
    const sealSize = 40; // Increased size for vehicle certificate
    pdf.addImage(
      sealDataUrl,
      "PNG",
      pageWidth - 85,
    ...
    console.warn("Failed to load vehicle certificate seal /chapa.png", e);
  }
  return pdf;
}
...
export async function generatePartnershipPDF(p: any) {
  ...
  try {
    // Official Partnership Seal (chapa.png)
    const sealUrl = "/chapa.png";
    const response = await fetch(sealUrl);
    const blob = await response.blob();
    const arrayBuffer = await blob.arrayBuffer();
    ...
    const sealSize = sealR * 2.5; // Slightly larger for prominence
    pdf.addImage(
      sealDataUrl,
      "PNG",
      sealX - sealSize / 2,
    ...
    console.warn("Failed to load partnership seal /chapa.png, falling back", e);
