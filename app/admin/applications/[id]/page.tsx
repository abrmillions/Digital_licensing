...
export default function ApplicationReview({
  ...
  const [verifiedOnlyTrue, setVerifiedOnlyTrue] = useState<boolean>(false);
      ...
      try {
        ...
        const [appData, settingsData] = await Promise.all([
          ...
          settingsApi.get().catch((err) => {
            ...
            return { documentVerificationEnabled: false, verifiedOnlyTrue: false };
          }),
        ]);
        ...
        setVerifiedOnlyTrue(!!settingsData.verifiedOnlyTrue);
        ...
        if (String(appData.status || "").toLowerCase() === "approved") {
          applicationsApi
            ...
            .catch((err) => {
              console.warn(
                `[admin] Failed to fetch license for application ${id}:`,
                err,
              );
        ...
        documentsApi
          ...
          .catch((err) => {
            console.warn(
              `[admin] Failed to fetch documents for application ${id}:`,
              err,
            );
            if (isMounted) setDocuments([]);
          });
      } catch (error: any) {
        // If it's a 404 (application deleted), handle it gracefully
        const is404 =
          error?.status === 404 ||
          error?.message?.includes("No Application matches the given query") ||
          error?.error?.detail?.includes("No Application matches the given query");

        if (is404) {
          console.debug(`[admin] Application ${id} not found (404)`);
          if (isMounted) {
            setApplication(null);
            setIsLoading(false);
            setShowSlowLoad(false);
        ...
        console.error(
          "[admin] Critical error loading application data:",
          error,
        );
        if (isMounted) {
          toast({
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      try {
        ...
        if (e?.status === 404) setApplication(null);
      }
      try {
  ...
  const handleVerifySingleDocument = async (docId: string) => {
    ...
    try {
      ...
      const newDocs = Array.isArray(docsData)
        ? docsData
        : docsData.results || [];
        ...
        {showSlowLoad && (
          <div className="flex flex-col items-center gap-2 animate-in fade-in duration-500">
            <p className="text-sm text-amber-600 font-medium text-center max-w-xs">
              This is taking longer than usual. The server might be slow or
              unreachable.
            </p>
            <Button
              variant="outline"
              ...
              <CardContent className="space-y-4">
                ...
                {(() => {
                  const photoPath =
                    ...
                    application.professional_photo ||
                    application.company_representative_photo ||
                    data.profile_photo ||
                  ...
                  const photoUrl = photoPath.startsWith("http")
                    ? photoPath
                    : `${DJANGO_API_URL}${photoPath.startsWith("/") ? "" : "/"}${photoPath}`;

                  return (
                    <div className="flex flex-col items-center sm:items-start gap-3 mb-4 pb-4 border-b">
                      <Label className="text-slate-500">Certificate Photo</Label>
                      <div className="relative h-32 w-32 overflow-hidden rounded-xl border-2 border-slate-200 shadow-sm">
                        <img
                          src={photoUrl}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Link
                        href={`/admin/users?search=${encodeURIComponent(application.applicant)}`}
                ...
                <div className="space-y-2">
                  {documents.length === 0 ? (
                    ...
                    documents.map((doc, index) => (
                      <div
                        key={index}
                        className="flex flex-col p-4 border rounded-xl bg-white hover:bg-slate-50 transition-colors shadow-sm gap-4"
                      >
                        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          <div className="flex items-start sm:items-center gap-3 min-w-0 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              <p
                                ...
                                title={doc.name || doc.file.split("/").pop()}
                              >
                          ...
                          <div className="flex items-center gap-2 shrink-0 w-full sm:w-auto mt-2 sm:mt-0 pt-2 sm:pt-0 border-t sm:border-t-0">
                            ...
                            {verifyEnabled && (
                              <Button
                                ...
                                className="flex-1 sm:flex-initial"
                                onClick={() =>
                                  handleVerifySingleDocument(doc.id)
                                }
                          ...
                          verifiedInSession.has(doc.id) && (
                            <div className="mt-2 space-y-3 border-t pt-4">
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {typeof doc.verification_score === "number" && (
                                  <span className="text-xs font-medium text-slate-600 bg-slate-100 px-2 py-0.5 rounded-full">
                                    Confidence:{" "}
                                    {(doc.verification_score * 100).toFixed(1)}%
                                  </span>
                                )}
                                {/* Display Priority Chips / Alerts */}
                                {(() => {
                                  try {
                                    const details =
                                      doc.verification_details || "";
                                    const parts =
                                      details.split(/Openrouter:\s*/);
                                    const jsonString =
                                      parts.length > 1
                                        ? parts[1].trim()
                                        : details.trim();
                                    ...
                                    const chips = computePriorityChips(
                                      doc.name || "General",
                                      data,
                                    );
                                    return (
                                      <>
                                        {chips.alerts.map((alert, i) => (
                                          <Badge
                                            key={i}
                                            className={`bg-${alert.color}-600 hover:bg-${alert.color}-700 animate-pulse`}
                                          >
                              ...
                              <div className="bg-slate-50 p-3 sm:p-5 rounded-xl border border-slate-200 w-full">
                ...
                <div className="flex flex-col gap-2">
                  <Button
                    ...
                    disabled={
                      status === "approved" ||
                      isApproving ||
                      (verifiedOnlyTrue && !allDocumentsVerified)
                    }
                  >
                    {isApproving ? (
                      <Loader2 className="w-4 h-4 mr-2 animate-spin" />
                  ...
                  {verifiedOnlyTrue && !allDocumentsVerified && (
                    <p className="text-[10px] text-amber-600 font-medium text-center">
                      Verification required for all documents before approval.
                    </p>
                  )}
                  {status === "approved" && (
