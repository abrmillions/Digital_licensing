...
import { Shield, Loader2, Info, Building2, FileText, ArrowLeft, Download, Check, X } from "lucide-react"
...
export default function AdminPartnershipDetailPage() {
  ...
  const [verifiedOnlyTrue, setVerifiedOnlyTrue] = useState(false)
  ...
  const loadData = async () => {
    try {
      ...
      try {
        ...
        setVerifiedOnlyTrue(!!s.verifiedOnlyTrue)
      } catch {}
      return p
    } catch (e: any) {
      const is404 = e?.status === 404 || e?.message?.includes("matches the given query")
      if (is404) {
        console.debug(`[admin] Partnership ${id} not found (404)`);
        setPartnership(null);
        return null;
      }
  ...
  const allDocumentsVerified =
    ...
    partnership.documents.length > 0 &&
    partnership.documents.every((doc: any) => doc.is_verified);
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <p className="text-xs font-medium text-slate-700">
                  Applicant: {partnership.owner_name || "-"}
                </p>
                {partnership.owner_email && (
                  <>
                    <span className="text-muted-foreground text-xs">•</span>
                    <Link 
                      href={`/admin/users?search=${encodeURIComponent(partnership.owner_email)}`}
