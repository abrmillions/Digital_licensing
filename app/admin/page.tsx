...
import { ClipboardList, Users, Building2, FileText, TrendingUp, Settings, Loader2, Mail, Bell } from "lucide-react"
    ...
    const checkAuthAndFetchStats = async () => {
      try {
        ...
        if (userStr) {
            ...
            if (role !== 'admin') {
                setError("Access Denied: Only administrator accounts can access this portal.")
                setIsLoading(false)
                return
            }
        }
        ...
        // Handle 403 specifically (logged in but not admin)
        if (error?.status === 403 || error?.response?.status === 403 || error?.message?.includes("Permission denied")) {
             setError("Access Denied: Only administrator accounts can access this portal.")
        } else if (error?.status === 401) {
             // Let the global djangoApiRequest handle the 401 redirect
             return;
        } else {
             toast({
                title: "Error",
