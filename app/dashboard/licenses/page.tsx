...
export default function MyLicenses() {
      ...
      console.warn("[clms] failed to migrate cached license to backend", c, e)
        ...
        const normalized = serverLicenses.map((lic: any) => {
          ...
          return {
            ...
            hasDownloaded: Boolean(d.downloaded || lic.downloaded || (lic.data && lic.data.downloaded)),
          }
        })
  ...
  const handleDownloadCertificate = async (license: any) => {
    ...
    if (license.status === "active" && license.canDownload !== false) {
      try {
        // Trigger backend download flag update and get latest data
        let latestLicense = license;
        if (license.backendId) {
          try {
            const response = await licensesApi.download(license.backendId)
            if (response && response.success && response.license) {
              latestLicense = response.license;
            }
          } catch (e) {
            console.warn("Failed to notify backend of download or fetch latest data", e)
          }
        }

        const { generateLicensePDF } = await import("@/lib/downloads/pdf-generator")
        const pdf = await generateLicensePDF(latestLicense)
        if (pdf) {
          pdf.save(`${license.type || "License"}-${license.licenseNumber || license.id}.pdf`)
          
          // Update local state to enable QR button immediately
          setLicenses(prev => prev.map(l => 
            l.backendId === license.backendId ? { ...l, hasDownloaded: true } : l
          ))
            ...
            {licenses.map((license) => (
              ...
              >
                ...
                <CardContent className="px-4 sm:px-5 pb-4 pt-0">
                  <div className="grid grid-cols-2 gap-y-3 gap-x-6 mb-4 p-3 bg-slate-50/50 rounded-lg border border-slate-100">
                    <div>
                        ...
                        <>
                          {showActions && (
                            <>
                              <Button 
                                size="sm" 
                                className="h-8 px-3 text-[11px] sm:text-xs font-bold shadow-sm" 
                                disabled={license.canDownload === false || !license.hasDownloaded}
                                 asChild={license.canDownload !== false && license.hasDownloaded}
                               >
                                 {license.canDownload !== false && license.hasDownloaded ? (
